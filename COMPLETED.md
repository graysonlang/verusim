...
- the active time rate and selected built-in scenario are mirrored in URL query parameters, override startup defaults on reload, preserve unrelated URL state, and omit the scenario parameter for file-backed content
...
URL-state regression coverage checks independent validation of scenario and time-rate overrides, preservation of unrelated query and fragment state, and removal of only the unreconstructable file-backed scenario selection.
...
Live browser verification, including the real-time movement, walking-calibration, and URL-state follow-ups, was unavailable because neither browser path had a reachable owner-run application and no isolated preview was authorized.
