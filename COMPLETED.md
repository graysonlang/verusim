...
World-view selection regression coverage distinguishes the nearest character hit from a Canvas background hit.
...
Another isolated Chromium pass selected an upstairs character from Exterior through a real Canvas hit, confirmed the Upper Floors projection, 100% focused camera, roster and inspector selection, and verified that a background hit cleared selection without leaving the active layer or producing console errors.
