...
It gives layer projection, selection clearing, canvas fitting, and sidebar visibility a coherent keyboard state machine without changing authoritative simulation state.
...
Status: reopened for workbench keyboard navigation.
...
Sidebar-layout regression coverage checks the 80-pixel close detent, 180-pixel open detent, viewport clamp, retained-width visibility toggles, keyboard resizing, and the three-state double-click cycle.
