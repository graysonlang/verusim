...
Status: reopened to separate handset hit targets from rendered control size.
...
- handset presentation keeps scenario identity and essential transport visible in a two-row safe-area-aware header, collapses the layer stack to its active control until opened, and preserves 44 CSS pixel hit targets while rendering primary square controls as centered 36-pixel elements
...
The final 390×844 handset Chromium pass measured the visible header, transport, rate, day-period, weather, zoom, layer, and roster-sheet controls at 40 pixels while retaining 12-pixel day information and full-bleed status art.
