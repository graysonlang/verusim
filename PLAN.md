...
Status: reopened for responsive control hierarchy refinement.
...
- handset transport retains the normal day-information type size and visually tighter controls inside the existing 44-pixel hit targets
- the signal verbosity and visibility selector occupies the character-roster title row rather than the application header
- the layer selector occupies the Canvas lower-left in every mode, while compact and handset zoom occupies the Canvas upper-right and handset zoom presents borderless value text without reducing its hit target
