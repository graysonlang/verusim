...
The workbench is two workspaces behind one persistent edit toggle - pressed means the scenario's authored drafts are open for editing (Build), released means the workbench is simulating - and they never share state.
Build (`app/workspace.ts`, presented by `app/build-workspace.ts`) owns the authoring graph - drafts, dirty state, transactional undo and redo - plus the selected document, property path, and editor viewport; Simulate owns the running state, its reset baseline, playback, interventions, runtime selection, and the world camera.
The only bridge is `prepareBuildRevision`, which runs the ordinary revision boundary and hands Simulate a prepared revision to start; nothing flows back, so interventions and snapshots cannot reach a draft and a draft edit cannot reach the running simulation.
Entering Build pauses playback, makes the Simulate panels inert, and hides the transport, rate, clock, day period, conditions, and world zoom, which all describe the running simulation; the transport actions are enabled only in Simulate; loading a scenario or file replaces both workspaces with a fresh project whose applied revision is the loaded one.
Build exists only in the wide layout (1080 pixels and up), where its explorer, editor, and inspector take the roster, stage, and inspector areas; the compact and handset layouts are Simulate-only, hide the edit toggle, disable the Build actions and shortcut, and return an open Build workspace to Simulate when the window narrows, keeping its drafts and history for when it widens again.
...
The canvas shares the simulation map's presentation model - the same corner layer stack, scale bar, zoom range, and camera operations - so the header zoom control, its menu, and the zoom and layer shortcuts drive whichever view is showing; the one shared problems pane under the editor rolls up every document's diagnostics and the last preparation failure and navigates to the owning document and field; it follows the sidebar model (`app/problems-pane.ts`) - collapsed to its header row with the count by default, resizable from its top edge with snap-closed and snap-open thresholds, double-click returning to the default height or collapsing when already there, arrow keys and Home on the edge, and the layout kept in device-local preferences.
`Control+Backquote` toggles the status bar and `Control+Shift+Backquote` the problems pane - Control rather than Command, which macOS reserves for window switching - and both chords work inside editable fields because they type nothing.
On the map and the layout canvas a plain wheel event (a trackpad's two-finger drag, or a mouse wheel) pans and a wheel event with `ctrlKey` (a trackpad pinch) zooms around the pointer.
