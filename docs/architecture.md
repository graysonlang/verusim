...
Entering Build pauses playback and makes the Simulate panels inert; the transport actions are enabled only in Simulate; loading a scenario or file replaces both workspaces with a fresh project whose applied revision is the loaded one.
