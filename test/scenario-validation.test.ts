...
describe('scenario validation', () => {
  ...
  it('migrates Phase 1 character and dyad content explicitly', () => {
    ...
    assert.equal(migratedScenario.schemaVersion, 16);
  ...
  it('migrates relational scenarios and snapshots to the agenda boundary', () => {
    ...
    assert.equal(migratedSnapshot.schemaVersion, 15);
    ...
    assert.equal(migratedSnapshot.agents[0]?.position.layerId, 'surface');
    assert.equal(migratedSnapshot.agents[0]?.destination.layerId, 'surface');
    ...
    assert.equal(migratedAgendaSnapshot.schemaVersion, 15);
