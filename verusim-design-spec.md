...
| 18 | ~~**Canonical time domain**~~ | **Resolved — Phase 9A (PLAN.md).** Integer simulated seconds, epoch-style, are canonical for state time, event resolution, deadlines, trace timestamps, and cadence scheduling; authored content and the §20.2 cadence table move to seconds with exact conversion of legacy minute content and snapshots. Sub-second precision, if ever needed, is a scale factor between solver ticks and observed seconds with matching rate scaling for movement and recovery - deferred, not built. |
| 19 | ~~**Trace retention contract**~~ | **Resolved — Phase 9A (PLAN.md).** Causal history is retained in per-character windows sized by narrative tier plus one shared window for entries with no character, never one global FIFO, so a noisy neighbor cannot evict a quiet character's causal sources. Entry identity is a per-character monotonic sequence independent of retention. Tier pools may later cap total memory above those floors but never replace them. |
| 20 | ~~**Character tiers**~~ | **Resolved — Phase 9A (PLAN.md).** A placement declares narrative salience as `principal`, `secondary`, or `background` (default `secondary` on migration). Tier sizes retention budgets and may inform host cadence policy; it is orthogonal to LOD cadence and never changes evaluator fidelity. |
| 21 | ~~**Runtime nomenclature**~~ | **Resolved — Phase 9A (PLAN.md).** Simulated people are characters at every surface. The runtime instance vocabulary aligns with the existing identity levels - `characterId`, `profileId`, `instanceId` - so runtime records address a character by `instanceId`; `agent` leaves the model, API, and storage vocabulary with the Phase 9A schema migration. |
